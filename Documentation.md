...
After the model is trained, a number of visualization options becomes available:
...
Calculates a number of relevant statistical properties, separated by training and test data. This includes the minimum and maximum error values, absolute mean square error, RMS error, etc.
...
Neural networks exist in a large variety of types and application scenarios. This tool only provides the functionality for once specific class of model, the Multilayer Perceptron. These are also often used for classification problems, this tool only works for regression problems however.

The tool can be used on an arbitrary number of inputs but only one output, which is the most used application scenario. Because this tool relies on the MLPRegressor method from the scikit-learn library, implementing more than one output is not possible at the time of creation.

Due to the relatively small amount of data and serial processing, no GPU acceleration is available for this tool (or scikit-learn in general). Fast CPU single core clock speeds help decreasing the training time.

Model optimization functionality is highliy limited due to the used MLPRegressor method. There is no pruning or dropout available. However tweaking the model parameters should still wield satisfactory results in the majority of cases.
...
- Mismatch in column count in training and test datasets or not enough columns
...
If **model initialization** fails, this is most likely caused by the parameter configuration. Double check the correct data types. Use period as decimal separator, not comma
